...
function changeSpeedOnBounce(){
  if(ball_x>=(paddle_width/2 + paddle_x)){
    ball_dx = min(ball_dx+0.7, 2.5);
  }
  else{
    ball_dx = min(ball_dx-0.7, 1.8);
  }
}

function bounce(){
  if ((ball_x>=paddle_x) && (ball_x<=(paddle_width+paddle_x)) && (ball_y >= (height-ball_diameter/2 - paddle_height)) &&(ball_y<(width-ball_diameter/2))){
    ball_dy *= -1;
    changeSpeedOnBounce();
